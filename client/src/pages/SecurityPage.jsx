...
const analyzeNetwork = (json) => {
  ...
  const weights={CRIT:-25,HIGH:-15,MED:-8,LOW:-3,INFO:0,PASS:0};let raw=50+(passedChecks/Math.max(totalChecks,1))*40;findings.forEach(f=>{raw+=weights[f.severity]||0;});const score=Math.max(5,Math.min(98,Math.round(raw)));const getLabel=s=>s>=80?"GOOD":s>=60?"MODERATE":s>=40?"AT RISK":"CRITICAL";const alerts={critical:findings.filter(f=>f.severity==="CRIT").length,high:findings.filter(f=>f.severity==="HIGH").length,medium:findings.filter(f=>f.severity==="MED").length,low:findings.filter(f=>f.severity==="LOW").length,pass:findings.filter(f=>f.severity==="PASS").length,info:findings.filter(f=>f.severity==="INFO").length};const getSev=cat=>findings.find(f=>f.category===cat)?.severity;const isPass=cat=>getSev(cat)==="PASS";const catHealth=[{name:"IP Addr",pct:isPass("IP VALIDATION")?100:35,color:isPass("IP VALIDATION")?"#10b981":"#f87171"},{name:"Protocols",pct:isPass("PROTOCOL SECURITY")?100:0,color:isPass("PROTOCOL SECURITY")?"#10b981":"#f87171"},{name:"ZBF",pct:isPass("ZONE FIREWALL")?90:20,color:isPass("ZONE FIREWALL")?"#10b981":"#f87171"},{name:"ACL",pct:isPass("ACL")?100:alerts.critical>0?0:40,color:isPass("ACL")?"#10b981":alerts.critical>0?"#f87171":"#f5c842"},{name:"AAA",pct:isPass("AAA / SESSION")?100:30,color:isPass("AAA / SESSION")?"#10b981":"#fb923c"},{name:"PKI/VPN",pct:isPass("PKI / CRYPTO")?80:20,color:isPass("PKI / CRYPTO")?"#10b981":"#f5c842"},{name:"Logging",pct:isPass("LOGGING")?100:15,color:isPass("LOGGING")?"#10b981":"#f5c842"},{name:"Segments",pct:isPass("SEGMENTATION")?100:50,color:isPass("SEGMENTATION")?"#10b981":"#f5c842"}];
...
const SEV={
  ...
  PASS:{label:"PASS",color:"#10b981",bg:"rgba(16,185,129,0.1)",border:"rgba(16,185,129,0.28)",icon:<CheckIcon/>},
...
const ScoreGauge=({score,label,animate})=>{
  ...
  const color=score>=80?"#10b981":score>=60?"#f5c842":score>=40?"#fb923c":"#f87171";
...
const RecommendationPanel=({finding})=>{
  ...
  if(recs.length===0)return(<div style={{display:"flex",alignItems:"center",gap:8,padding:"12px",background:"rgba(16,185,129,0.06)",border:"1px solid rgba(16,185,129,0.2)",borderRadius:8,fontSize:12,color:"rgba(255,255,255,0.55)"}}><CheckIcon/> No additional Cisco commands required.</div>);
  return(<div style={{display:"flex",flexDirection:"column",gap:10}}><div style={{display:"flex",alignItems:"center",gap:8,padding:"8px 12px",background:"rgba(16,185,129,0.08)",border:"1px solid rgba(16,185,129,0.18)",borderRadius:8}}><TerminalIcon/><span style={{fontSize:11,fontFamily:"'JetBrains Mono',monospace",color:"rgba(255,255,255,0.7)"}}>Cisco IOS Remediation Commands</span><span style={{marginLeft:"auto",fontSize:10,fontFamily:"'JetBrains Mono',monospace",padding:"2px 8px",borderRadius:4,background:"rgba(16,185,129,0.12)",border:"1px solid rgba(16,185,129,0.25)",color:"#10b981"}}>PDF Reference</span></div>{recs.map((rec,ri)=>(<div key={ri} style={{background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:8,overflow:"hidden"}}><div style={{padding:"8px 12px",borderBottom:"1px solid rgba(255,255,255,0.05)",display:"flex",alignItems:"center",gap:8}}><span style={{display:"inline-flex",alignItems:"center",gap:4,padding:"2px 7px",borderRadius:4,background:"rgba(16,185,129,0.1)",border:"1px solid rgba(16,185,129,0.2)",fontSize:9,fontFamily:"'JetBrains Mono',monospace",color:"#10b981",letterSpacing:"0.8px"}}><LayerIcon/>{rec.layer}</span><span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:11,fontWeight:700,color:"rgba(255,255,255,0.85)"}}>{rec.title}</span></div><div style={{padding:"8px 12px",display:"flex",flexDirection:"column",gap:4}}>{rec.steps.map((step,si)=>{const key=`${ri}-${si}`;const isOpen=openStep===key;return(<div key={si} style={{borderRadius:7,overflow:"hidden",border:"1px solid rgba(255,255,255,0.05)"}}><div onClick={()=>setOpenStep(isOpen?null:key)} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 10px",cursor:"pointer",background:isOpen?"rgba(16,185,129,0.06)":"rgba(255,255,255,0.01)"}}><span style={{width:20,height:20,borderRadius:5,background:"rgba(16,185,129,0.15)",border:"1px solid rgba(16,185,129,0.25)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:700,fontFamily:"'JetBrains Mono',monospace",color:"#10b981",flexShrink:0}}>{si+1}</span><span style={{flex:1,fontSize:12,color:"rgba(255,255,255,0.7)"}}>{step.label}</span><ChevronIcon open={isOpen}/></div>{isOpen&&<div style={{padding:"0 10px 10px"}}><CodeBlock code={step.code}/></div>}</div>);})}</div></div>))}</div>);
};
...
const SecurityPage = () => {
  ...
  const S = {
    ...
    btn: (primary) => ({display:"inline-flex",alignItems:"center",gap:7,padding:"7px 16px",borderRadius:8,fontSize:13,fontWeight:600,fontFamily:"'Inter',sans-serif",cursor:"pointer",border:"1px solid",transition:"all 0.18s",whiteSpace:"nowrap",background:primary?"linear-gradient(180deg,#10b981,#0d9668)":"rgba(255,255,255,0.04)",color:primary?"#fff":"rgba(255,255,255,0.8)",borderColor:primary?"rgba(16,185,129,0.55)":"rgba(255,255,255,0.08)",boxShadow:primary?"0 10px 26px rgba(16,185,129,0.18)":"none"}),
  };
    ...
    <div style={S.page}>
      ...
      <div style={S.topbar}>
        <div style={{display:"flex",alignItems:"center",gap:14}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8,color:"#10b981"}}>
            <ShieldIcon/>
            <span style={{fontWeight:700,fontSize:14,letterSpacing:0,color:"#f4f4f5"}}>StructuraNet <span style={{color:"#34d399"}}>AI</span></span>
            <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:11,letterSpacing:"1.2px",color:"rgba(255,255,255,0.42)"}}>SECURITY</span>
            <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:10,padding:"2px 7px",borderRadius:4,background:"rgba(16,185,129,0.14)",border:"1px solid rgba(16,185,129,0.28)",color:"#34d399"}}>v2.0</span>
            <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:9,padding:"2px 7px",borderRadius:4,background:"rgba(16,185,129,0.1)",border:"1px solid rgba(16,185,129,0.2)",color:"#10b981",marginLeft:4}}>L1→L7</span>
          </div>
        </div>
      ...
      {phase==="running"&&(
        <div style={{position:"relative",height:3,background:"rgba(255,255,255,0.04)",flexShrink:0}}>
          <div style={{height:"100%",background:"linear-gradient(90deg,#10b981,#34d399)",width:`${progress}%`,transition:"width 0.12s linear"}}/>
          <div style={{position:"absolute",top:-4,left:`${progress}%`,width:16,height:11,borderRadius:"50%",background:"#34d399",filter:"blur(5px)",transform:"translateX(-50%)",pointerEvents:"none"}}/>
        </div>
      )}

      {/* ── STATUS BAR ── */}
      <div style={{display:"flex",alignItems:"center",gap:10,padding:"5px 20px",background:"rgba(10,11,13,0.82)",borderBottom:"1px solid rgba(255,255,255,0.08)",fontFamily:"'JetBrains Mono',monospace",fontSize:11,color:"rgba(255,255,255,0.38)",flexShrink:0,flexWrap:"wrap"}}>
        ...
        {phase==="done"&&report&&(<>
          <span style={{color:"#10b981",fontWeight:600}}>Analysis complete</span>
          ...
          <span style={{opacity:0.25}}>·</span><span style={{color:"#10b981"}}>Pass: {report.alerts.pass}</span>
        </>)}
      </div>
        ...
        <div style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:24,padding:"24px 28px",scrollbarWidth:"thin",scrollbarColor:"rgba(255,255,255,0.08) transparent"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:10,textAlign:"center"}}>
            <div style={{display:"flex",alignItems:"center",gap:10,color:"#10b981"}}>
              <ShieldIcon/>
              <span style={{fontFamily:"'JetBrains Mono',monospace",fontWeight:700,fontSize:18,letterSpacing:1.2,color:"#f4f4f5"}}>Topology Security Analyzer</span>
            </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(190px,1fr))",gap:10,width:"100%",maxWidth:760}}>
            {[
              ...
              <div key={title} style={{background:"rgba(255,255,255,0.035)",border:"1px solid rgba(16,185,129,0.16)",borderRadius:10,padding:"12px 14px",textAlign:"left"}}>
                <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:10,letterSpacing:"1px",color:"#34d399",marginBottom:6}}>{title.toUpperCase()}</div>
      ...
      {phase==="running"&&(
        <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:18}}>
          ...
          <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:28,fontWeight:700,color:"#10b981"}}>{Math.round(progress)}%</div>
        </div>
      )}
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(190px,1fr))",gap:12}}>
            ...
            <div style={S.card}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,textAlign:"center"}}>
                {[{n:report.alerts.critical,l:"CRITICAL",c:"#f87171"},{n:report.alerts.high,l:"HIGH",c:"#fb923c"},{n:report.alerts.medium,l:"MEDIUM",c:"#f5c842"},{n:report.alerts.low||0,l:"LOW",c:"#60a5fa"},{n:report.alerts.pass,l:"PASS",c:"#10b981"},{n:report.alerts.info||0,l:"INFO",c:"#94a3b8"}].map(a=>(<div key={a.l} style={{display:"flex",flexDirection:"column",gap:3}}><span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:22,fontWeight:700,color:a.c}}>{a.n}</span><span style={{fontSize:9,letterSpacing:"0.8px",color:"rgba(255,255,255,0.38)"}}>{a.l}</span></div>))}
              </div>
            </div>
            <div style={S.card}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"260px 1fr",gap:14,alignItems:"start"}}>
            <div style={{display:"flex",flexDirection:"column",gap:12}}>
              <div style={S.card}>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:10}}>
                  {[{k:"CRIT",l:"CRITICAL",c:"#f87171",n:report.alerts.critical},{k:"HIGH",l:"HIGH",c:"#fb923c",n:report.alerts.high},{k:"MED",l:"MEDIUM",c:"#f5c842",n:report.alerts.medium},{k:"LOW",l:"LOW",c:"#60a5fa",n:report.alerts.low||0},{k:"INFO",l:"INFO",c:"#94a3b8",n:report.alerts.info||0},{k:"PASS",l:"PASS",c:"#10b981",n:report.alerts.pass}].map((s,i)=>(
            ...
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              ...
              <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                {[{key:"ALL",label:`All (${sevCounts.ALL})`},{key:"CRIT",label:`Critical (${sevCounts.CRIT})`,color:"#f87171"},{key:"HIGH",label:`High (${sevCounts.HIGH})`,color:"#fb923c"},{key:"MED",label:`Medium (${sevCounts.MED})`,color:"#f5c842"},{key:"INFO",label:`Info (${sevCounts.INFO})`,color:"#94a3b8"},{key:"PASS",label:`Pass (${sevCounts.PASS})`,color:"#10b981"}].map(tab=>(
                  <button key={tab.key} onClick={()=>setFilter(tab.key)} style={{padding:"5px 13px",borderRadius:7,fontSize:11.5,fontFamily:"'JetBrains Mono',monospace",cursor:"pointer",border:filter===tab.key?`1px solid ${tab.color||"#10b981"}`:"1px solid rgba(255,255,255,0.07)",background:filter===tab.key?"rgba(255,255,255,0.05)":"rgba(255,255,255,0.02)",color:filter===tab.key?(tab.color||"#10b981"):"rgba(255,255,255,0.38)",transition:"all 0.15s"}}>{tab.label}</button>
                ))}
              </div>
